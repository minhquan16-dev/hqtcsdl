...
5. **Dọn dẹp tự động (đa luồng)** — Xóa tin hết hạn, kiểm tra URL tin bị gỡ trên TopCV với số luồng tùy chọn, dọn bản ghi mồ côi.
...
Sau khi chạy lệnh, chương trình sẽ hỏi nhập **số luồng** để kiểm tra tin bị gỡ (cơ chế hoạt động giống đa luồng khi cào):

```
==================================================
🧹 CHẾ ĐỘ DỌN DẸP DATABASE
==================================================

📅 Bước 1: Xóa tin đã hết hạn nộp hồ sơ...

🌐 Bước 2: Kiểm tra tin bị gỡ trên TopCV...
Nhập số luồng để kiểm tra (mặc định 1): 3
→ Sử dụng 3 luồng để kiểm tra tin bị gỡ.
```

- Nhấn **Enter** → sử dụng 1 luồng (như cũ)
- Nhập số bất kỳ (VD: `3`, `5`) → mở bấy nhiêu cửa sổ Chrome song song, chia đều danh sách tin cho các luồng theo round-robin
...
| 2 | **Xóa tin bị gỡ (đa luồng)** — Mỗi luồng mở 1 Chrome riêng, truy cập URL trên TopCV, kiểm tra 404/redirect (xác nhận 3 lần trước khi xóa). Tự restart Chrome khi crash (tối đa 5 lần/luồng) |
...
- `init_lock` đồng bộ hóa việc khởi tạo Chrome driver (cả crawl lẫn cleanup)
- `cleanup_counter_lock` bảo vệ biến đếm số tin đã xóa/bỏ qua giữa các luồng cleanup
...
- Quá trình crawl/cleanup mở nhiều cửa sổ Chrome, cần đủ RAM (~4GB trở lên khuyến nghị)
